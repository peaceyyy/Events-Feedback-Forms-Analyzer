"""
Test script for Aqua Forms CSV extraction and column mapping analysis.

This script:
1. Loads the actual Aqua forms CSV from test_data
...
def load_aqua_forms_csv():
    """Load the actual Aqua forms CSV from test_data."""
    ...
    if not os.path.exists(csv_path):
        print(f"❌ Aqua forms CSV not found at: {csv_path}")
        return None
    
    df = pd.read_csv(csv_path)
    print(f"✅ Loaded Aqua forms CSV: {len(df)} rows, {len(df.columns)} columns")
    return df
...
def analyze_column_mapping(aqua_df):
    """
    Analyze which Aqua columns can map to existing expected columns.
    ...
    aqua_columns = aqua_df.columns.tolist()
    
    # Manual mapping based on inspection
    # Format: 'expected_key': ('Aqua Column Name', 'transformation_needed')
    proposed_mapping = {
    ...
    # Columns that exist in Aqua but don't map to current system
    aqua_specific_columns = [
        'First Name',
        'Last Name', 
        'Email',
        'Responded',
    ...
    # Columns expected by current system but NOT in Aqua
    missing_in_aqua = [
        'sessions_attended',  # No session breakdown in Aqua forms
    ...
    print("-" * 80)
    for key, (aqua_col, transformation) in proposed_mapping.items():
        current_name = CURRENT_SYSTEM_COLUMNS.get(key, 'N/A')
        print(f"\n  {key}:")
        print(f"    Current system expects: {current_name}")
        print(f"    Aqua column: {aqua_col}")
        print(f"    Transformation: {transformation}")
    
    print("\n\n❌ COLUMNS MISSING IN AQUA FORMS (will cause errors):")
    print("-" * 80)
    for col in missing_in_aqua:
        print(f"  - {col}: {CURRENT_SYSTEM_COLUMNS[col]}")
    
    print("\n\n🆕 AQUA-SPECIFIC COLUMNS (new data available):")
    print("-" * 80)
    print("  Metadata columns (useful for filtering/grouping):")
    metadata_cols = [c for c in aqua_specific_columns if c in [
    ...
    insight_cols = [c for c in aqua_specific_columns if 'familiar' in c.lower() 
                    or 'implement' in c.lower() or 'included' in c.lower()]
    for col in insight_cols:
        print(f"    - {col}")
    
    return proposed_mapping, missing_in_aqua, aqua_specific_columns


def test_data_extraction(aqua_df, proposed_mapping):
    """
    Test extracting and transforming data from Aqua forms using proposed mapping.
    """
    print("\n\n" + "="*80)
    ...
    # Sample first 3 rows
    sample_size = min(3, len(aqua_df))
    
    for idx in range(sample_size):
        print(f"\n--- Sample Row {idx + 1} ---")
        row = aqua_df.iloc[idx]
        
        for key, (aqua_col, transformation) in proposed_mapping.items():
            raw_value = row.get(aqua_col, 'N/A')
...
def suggest_integration_strategy():
    """
    Print recommendations for integrating Aqua forms without breaking existing system.
    """
    print("\n\n" + "="*80)
...
---------------------------------------------------------
Create a new extractor: `extract_aqua_forms_data()` in feedback_service.py
- Keep existing `extract_feedback_data()` unchanged
- New function handles Aqua-specific column mapping
...
- If columns match Aqua → use new mapping
...
- Define multiple schema presets (test_csv, aqua_forms, custom)
...
CHARTS THAT WILL BREAK WITH AQUA FORMS:
----------------------------------------
...
NEW CHARTS POSSIBLE with Aqua data:
...
------------------------------
1. Create `extract_aqua_forms_data()` separate function
2. Add a `/api/upload/aqua` endpoint or form-type parameter
3. Update frontend to conditionally render only compatible charts
4. Add a "form type" indicator in upload response
5. For future forms, add the missing questions to Aqua template
...
def main():
    print("=" * 80)
    print("AQUA FORMS CSV EXTRACTION & MAPPING ANALYSIS")
    print("=" * 80)
    
    # Load both CSVs
    aqua_df = load_aqua_forms_csv()
    test_df = load_test_csv()
    
    if aqua_df is None:
        print("\n❌ Cannot proceed without Aqua forms CSV")
        return
    ...
    print("\n📊 AQUA FORMS CSV COLUMNS:")
    print("-" * 80)
    for col in aqua_df.columns:
        print(f"  - {col}")
    
    # Analyze mapping
    proposed_mapping, missing_cols, aqua_specific = analyze_column_mapping(aqua_df)
    
    # Test extraction
    test_data_extraction(aqua_df, proposed_mapping)
