...
export default function VenueModalityPreferencesChart({
  ...
  const modalityData = React.useMemo(() => {
    ...
    return data.data.modality_breakdown.map((item: any) => ({
      ...
      value: item.count,
      percentage: item.percentage,
      fill: item.modality === "Online" ? "#ea4335" : "#4CAF50",
    }));
  }, [data]);
    ...
    return (
      <div className="glass-card-dark p-3 rounded-lg border border-white/20">
        ...
        <div className="space-y-1 text-xs">
          ...
          <p style={{ color: "var(--color-text-secondary)" }}>
            Responses: <span className="font-semibold">{item.value}</span> (
            {item.percentage}%)
          </p>
          {item.satisfaction && (
            <p style={{ color: "var(--color-text-secondary)" }}>
  ...
  return (
    <div className={`glass-card ${className}`}>
      {/* Chart */}
      <div
        className="glass-card-dark p-4 rounded-lg mb-4"
