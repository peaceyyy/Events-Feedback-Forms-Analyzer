// components/analysis/textAnalytics/AIInsightsContainer.tsx
'use client'
...
export default function AIInsightsContainer({ 
  ...
  if (!aiResults && !loading) {
    return (
      <div className="glass-card-dark p-8 rounded-xl text-center">
        ...
        <button
          onClick={generateAIInsights}
          className="px-6 py-3 bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 text-white rounded-lg font-medium transition-all transform hover:scale-105 flex items-center justify-center gap-2"
        >
          <AIIcon sx={{ fontSize: 20 }} /> Generate AI Insights
        </button>
  ...
  return (
    <div className="space-y-8">
      {/* AI Analysis Header */}
      <div className="glass-card-dark p-6 rounded-xl text-center bg-gradient-to-r from-blue-500/10 to-purple-500/10">
        <AIIcon sx={{ fontSize: 32, color: 'var(--color-google-blue)' }} />
        <h2 className="text-2xl font-bold mt-2" style={{ color: 'var(--color-text-primary)' }}>
          AI-Generated Insights
        </h2>
        <p style={{ color: 'var(--color-text-secondary)' }}>
          Powered by Google Gemini • Analysis complete
          {aiResults?.sentiment?.dev_mode && (
            <span className="ml-2 px-2 py-1 bg-orange-500/20 text-orange-300 rounded text-xs">
              DEV MODE - Smaller samples
            </span>
          )}
        </p>
        <button
          onClick={generateAIInsights}
          disabled={loading}
          className="mt-3 px-4 py-2 bg-blue-600/20 hover:bg-blue-600/30 text-blue-300 rounded-lg text-sm transition-colors"
        >
          Refresh Analysis
        </button>
      </div>
      ...
      <div className="text-center">
        ...
        >
          {loading ? 'Analyzing...' : 'Regenerate AI Insights'}
        </button>
      </div>
    </div>
  )
